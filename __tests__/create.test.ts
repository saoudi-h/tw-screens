...
const { useScreen, useScreenReverse, useScreenEffect, useScreenValue, useBreakpointManager } =
    create(screens)
...
describe('useScreenValue', () => {
    it('should return valid value when screen matches', () => {
        ...
        const { result } = renderHook(() => useScreenValue('md', 'matched', 'not matched'))
...
describe('Default screens functionality', () => {
    ...
    it('should return correct values with useScreenValue for default screens', () => {
        setScreenWidth(640)
        const { result } = renderHook(() => useDefaultScreenValue('sm', 'matched', 'not matched'))
        expect(result.current).toBe('matched')
    })
})
