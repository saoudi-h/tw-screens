...
export function generateMediaQuery(screenValue: ScreenValue): string {
    try {
        if (screenValue == null) {
            throw new MediaQueryError('Screen value cannot be null or undefined', screenValue)
        }
        ...
        if (typeof screenValue === 'object') {
            ...
            const hasValidProperty = Object.keys(screenValue).some(key => validProperties.has(key))
            if (!hasValidProperty) {
                throw new MediaQueryError(
            ...
            if ('raw' in screenValue && screenValue.raw != null) {
                if (typeof screenValue.raw !== 'string') {
                    throw new MediaQueryError('Raw value must be a string', screenValue.raw)
                }
            ...
            if ('min' in screenValue && screenValue.min != null) {
                if (typeof screenValue.min !== 'string') {
                    throw new MediaQueryError('Min value must be a string', screenValue.min)
                }
            ...
            if ('max' in screenValue && screenValue.max != null) {
                if (typeof screenValue.max !== 'string') {
                    throw new MediaQueryError('Max value must be a string', screenValue.max)
                }
            ...
            if (conditions.length === 0) {
                throw new MediaQueryError('No valid conditions found in screen object', screenValue)
            }
        ...
        throw new MediaQueryError('Unexpected error while generating media query', screenValue)
    }
}
